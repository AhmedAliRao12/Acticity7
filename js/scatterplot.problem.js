...
legends_items
    .on("click", (event, d) => {
        ...
        if (activeCountries.size === 0) {
            ...
            legends_items.select("rect")
              .attr("stroke", "none")
              .attr("stroke-width", 0);
        } else {
            d3.selectAll("circle")
              .attr("opacity", p => activeCountries.has(p[colorCol]) ? 1 : 0.1);
            legends_items.select("rect")
              .attr("stroke", c => activeCountries.has(c) ? "black" : "none")
              .attr("stroke-width", c => activeCountries.has(c) ? 2 : 0);
        }
    });

    legends_items.style("pointer-events", "all");   
    legends_items
        .append("text")
        .text(d=>d)
        .attr("dx",45)
